...
class LabelBiasDetector:
    ...
    def _check_disparate_impact(self, data: pd.DataFrame, attr: str) -> list[BiasFindings]:
    ...
    def _check_statistical_parity(self, data: pd.DataFrame, attr: str) -> list[BiasFindings]:
        ...
        if spd > self.thresholds.statistical_parity_critical:
            ...
            findings.append(
                BiasFindings(
                    ...
                    description=(f"Positive outcome rates differ by {spd:.1%} between groups."),
                    affected_attribute=attr,
                    affected_groups=[
                        str(spd_result["privileged_group"]),
                        str(spd_result["unprivileged_group"]),
    ...
    def _check_label_rate_significance(self, data: pd.DataFrame, attr: str) -> list[BiasFindings]:
        """Check if label rate differences are statistically significant."""
        findings = []

        # Get label rates by group
        rates = group_label_rates(data, attr, self.config.target_column, self.config.positive_label)

        # Perform chi-squared test
        contingency = pd.crosstab(
            data[attr], data[self.config.target_column] == self.config.positive_label
        )
    ...
    def _check_intersectional_label_bias(self, data: pd.DataFrame) -> list[BiasFindings]:
        ...
        for i, attr1 in enumerate(attrs):
            for attr2 in attrs[i + 1 :]:
                ...
                intersection_rates = data.groupby(intersection)[self.config.target_column].apply(
