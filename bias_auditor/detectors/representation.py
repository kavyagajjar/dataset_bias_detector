...
class RepresentationDetector:
    ...
    def _check_group_proportions(self, data: pd.DataFrame, attr: str) -> list[BiasFindings]:
    ...
    def _check_imbalance(self, data: pd.DataFrame, attr: str) -> list[BiasFindings]:
    ...
    def _check_reference_distribution(self, data: pd.DataFrame, attr: str) -> list[BiasFindings]:
    ...
    def _check_intersectional(self, data: pd.DataFrame, primary_attr: str) -> list[BiasFindings]:
        """Check for intersectional underrepresentation."""
        findings = []

        other_attrs = [
            a for a in self.config.protected_attributes if a != primary_attr and a in data.columns
        ]
        ...
        for other_attr in other_attrs[: self.config.max_intersectional_depth - 1]:
            ...
            small_groups = intersection_counts[intersection_counts < self.config.min_group_size]
